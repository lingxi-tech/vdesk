...
This document describes the product requirements for a web-based application to manage docker containers on a server.
## Overview
The application shall provide a user-friendly interface to create, manage, and monitor docker containers running remote desktops. The application shall consist of a frontend developed in Vue and a backend developed in Python using FastAPI. The backend shall interact with the Docker CLI to manage the containers.
